//types.js
const GET_PRODUCTS = "GET_PRODUCTS";
const GET_PRODUCTS_ADMIN = "GET_PRODUCTS_ADMIN";
const GET_ORDERS_ADMIN = "GET_ORDERS_ADMIN";
const GET_DETAIL = "GET_DETAIL";
const SET_IMAGE_URL = "SET_IMAGE_URL";
const POST_PRODUCT = "POST_PRODUCT";
const GET_PRODUCT_TYPE = "GET_PRODUCT_TYPE";
const GET_PRODUCT_TYPE_BYID = "GET_PRODUCT_TYPE_BYID";
const GET_COLOR = "GET_COLOR";
const GET_COLOR_BYID = "GET_COLOR_BYID";
const GET_MATERIAL = "GET_MATERIAL";
const GET_MATERIAL_BYID = "GET_MATERIAL_BYID";
const GET_PRODUCT_BY_NAME = "GET_PRODUCT_BY_NAME";
const GET_CART = "GET_CART";
const SET_SORT = "SET_SORT";
const SET_PRODUCT_TYPE = "SET_PRODUCT_TYPE";
const SET_COLOR = "SET_COLOR";
const SET_PRICE_RANGE = "SET_PRICE_RANGE";
const SET_PRODUCTS_COPY = "SET_PRODUCTS_COPY";
const POST_CART_PRODUCT = "POST_CART_PRODUCT";
const DELETE_CART_PRODUCT = "DELETE_CART_PRODUCT";
const DELETE_CART = "DELETE_CART";
const POST_USER = "POST_USER";
const LOGIN = "LOGIN";
const LOAD_CART_FROM_LOCAL_STORAGE = "LOAD_CART_FROM_LOCAL_STORAGE";
const POST_CART = "POST_CART";
const LOGOUT = "LOGOUT";
const FETCH_USER_DATA = "FETCH_USER_DATA";
const SET_MATERIAL = "SET_MATERIAL";
const GET_USERS = "GET_USERS";
const UPDATE_CART = "UPDATE_CART";
const PUT_PRODUCT = "PUT_PRODUCT";
const DELETE_PRODUCT = "DELETE_PRODUCT";
const ADMIN_ENABLEDISABLE = "ADMIN_ENABLEDISABLE";
const SET_NAME = "SET_NAME";
const POST_COLOR = "POST_COLOR";
const POST_MATERIAL = "POST_MATERIAL";
const POST_PRODUCTTYPE = "POST_PRODUCTTYPE";
const DELETE_CART_PRODUCT_DIRECT = "DELETE_CART_PRODUCT_DIRECT";
const EMPTY_CART = "EMPTY_CART";
const POST_REVIEW = "POST_REVIEW";
const GET_CARTS = "GET_CARTS";
const GET_REVIEW_BY_PRODUCT_ID = "GET_REVIEW_BY_PRODUCT_ID";
const GET_REVIEWS = "GET_REVIEWS";
const DELETE_REVIEW = "DELETE_REVIEW";

module.exports = {
  GET_CARTS,
  GET_PRODUCTS,
  GET_PRODUCTS_ADMIN,
  GET_ORDERS_ADMIN,
  GET_DETAIL,
  GET_USERS,
  SET_IMAGE_URL,
  POST_PRODUCT,
  GET_PRODUCT_TYPE,
  GET_PRODUCT_TYPE_BYID,
  GET_COLOR,
  GET_COLOR_BYID,
  GET_MATERIAL,
  GET_MATERIAL_BYID,
  GET_PRODUCT_BY_NAME,
  GET_CART,
  SET_SORT,
  SET_PRODUCT_TYPE,
  SET_PRODUCTS_COPY,
  SET_COLOR,
  SET_NAME,
  SET_PRICE_RANGE,
  POST_CART_PRODUCT,
  DELETE_CART_PRODUCT,
  DELETE_CART,
  DELETE_PRODUCT,
  POST_USER,
  LOGIN,
  LOGOUT,
  FETCH_USER_DATA,
  LOAD_CART_FROM_LOCAL_STORAGE,
  POST_CART,
  SET_MATERIAL,
  PUT_PRODUCT,
  ADMIN_ENABLEDISABLE,
  POST_COLOR,
  POST_MATERIAL,
  POST_PRODUCTTYPE,
  UPDATE_CART,
  DELETE_CART_PRODUCT_DIRECT,
  EMPTY_CART,
  POST_REVIEW,
  GET_REVIEW_BY_PRODUCT_ID,
  GET_REVIEWS,
  DELETE_REVIEW,
};
